        ...
        public ShipmentModel ProcessShipment(ShipmentModel model)
        {
            ...
            for (int i = 1; i <= NumberOfRetries; i++)
            {
                try
                {
                    ...
                    if (model.PackagingType == "21")
                    {
                        ptype = "UPS Express Box";
                    }
                    else if (model.PackagingType == "04")
                    {
                        ptype = "UPS PAK";
                    }
                    else if (model.PackagingType == "03")
                    {
                        ptype = "UPS Tube";
                    }
                    else if (model.PackagingType == "02")
                    {
                        ptype = "Other Packaging";
                    }
                    else if (model.PackagingType == "01")
                    {
                        ptype = "UPS Express Envelope";
                    }

                    stype = model.ReviewServiceName;

                    model.Result = "<p>The transaction was a " + shipmentResponse.Response.ResponseStatus.Description.ToLowerInvariant() + ".</p>";
                    //model.Result += "<p>The 1Z number of the new shipment is " + shipmentResponse.ShipmentResults.ShipmentIdentificationNumber + ".</p>";
                    model.Result += "<p>Thank you for choosing Vital Logistics Ltd.</p>";
                    model.Result += "<p>Shipment Details:</p>";
                    model.Result += "<table><tr><td>Tracking Number:</td><td>" + shipmentResponse.ShipmentResults.ShipmentIdentificationNumber + "</td></tr><tr><td>Service Type:</td><td>" + stype + "</td></tr><tr><td>Packaging Type:</td><td>" + ptype + "</td></tr></table>";
