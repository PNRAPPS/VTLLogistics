        ...
        public TNTResponse GetTimeInTransit(ShipmentModel data)
        {
            ...
            for (int i = 1; i <= NumberOfRetries; i++)
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(data.PackageDeclaredValueCAD))
                    {
                        var invoice = new InvoiceLineTotal();
                        
                        invoice.CurrencyCode = "CAD";
                        invoice.MonetaryValue = data.PackageDeclaredValueCAD;

                        tntRequest.InvoiceLineTotal = invoice;
                    }
                    else
                    {
                        var invoice = new InvoiceLineTotal();
