    ...
    public class ShipmentDraftService : IShipmentDraftService
    {
        ...
        public void UpdateShipmentDraft(Model.ShipmentModel model, Guid id)
        {
            ...
            if (!model.IsSuccess)
            {
                data.Status = "Draft";
                data.DateModified = DateTime.Now;
                data.IsSuccess = model.IsSuccess.ToString();
                data.ResultMessage = model.ResultMessage;
            }
        ...
        public Model.ShipmentModel DataToModel(ShipmentDraft data)
        {
            ...
            model.IsSuccess = string.IsNullOrEmpty(data.IsSuccess) ? true : Convert.ToBoolean(data.IsSuccess);
            model.ResultMessage = data.ResultMessage;
