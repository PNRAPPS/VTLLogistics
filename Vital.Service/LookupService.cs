...
namespace Vital.Service
{
    public class LookupService : ILookupService
    {
        ...
        {
            var data = _lookupRespository.Query().Get().First(r => r.LookupCode.Equals(code) && r.LookupValue.Equals(value)).LookupText;
            return data;
        }

        public string GetLookupValue(string code, string text)
        {
            var data = _lookupRespository.Query().Get().FirstOrDefault(r => r.LookupCode.Equals(code) && r.LookupText.Equals(text));
            return data == null ? string.Empty : data.LookupValue;
        }
